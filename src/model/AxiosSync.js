...
export default (axios) => {
  return (method, model, optionsInput) => {
    ...
    const options = _.assign({}, optionsInput);
    ...
    if (ajaxConfig.headers) {
      for (key in ajaxConfig.headers) { // eslint-disable-line
        headers[key.toLowerCase()] = ajaxConfig.headers[key];
      }
    }
    if (options.headers) {
      for (key in options.headers) { // eslint-disable-line
    ...
    if (ajaxConfig.xhrFields) {
      let beforeSend = ajaxConfig.beforeSend;
      params.beforeSend = (req) => {
        _.assign(req, ajaxConfig.xhrFields);
        if (beforeSend) {
          return beforeSend.apply(this, [method, model, optionsInput]);// eslint-disable-line
        }
      };
    ...
    let ajaxSettings = _.assign(params, options);
    ...
    let promise = new Promise((resolve, reject) => {
      request = axios.request(ajaxSettings).then((resp) => {
        ...
        if (options.error) {
          ...
          let message = (err ? err.message : (body || 'HTTP'
              + resp.statusCode));
    ...
    if (model) {
      model.trigger('request', model, request, optionsInput, ajaxSettings);
    }
